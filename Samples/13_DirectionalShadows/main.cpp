...
namespace Oksijen
{
	...
	struct MeshViewProjectionBuffer
	{
		glm::mat4x4 View;
		glm::mat4x4 Projection;
	};
	struct MeshModelBuffer
	{
		glm::mat4x4 Model;
	};
	...
	struct RenderableObject
	{
		...
		GraphicsBuffer* pViewProjectionBuffer = nullptr;
		GraphicsBuffer* pViewProjectionStageBuffer = nullptr;
		GraphicsBuffer* pModelBuffer = nullptr;
		GraphicsBuffer* pModelStageBuffer = nullptr;
		DescriptorSet* pViewportProjectionDescriptorSet = nullptr;
		DescriptorSet* pModelDescriptorSet = nullptr;
	};
	struct MaterialData
	{
	...
	static const char shadowVertexShaderSource[] =
		R"STR(
		#version 430 core
			...
			layout(std140,binding = 0,set = 0) uniform ShadowLightBuffer
			{
				mat4 View;
				mat4 Projection;
			};
			layout(std140,binding = 0,set = 1) uniform ModelBuffer
			{
				mat4 Model;
			};
			...
			void main()
			{
				gl_Position = vec4(aPos,1.0f)*transpose(Projection*View*Model);
			}
			
		)STR";
	static const char shadowFragmentShaderSource[] =
		R"STR(
		#version 430 core
	...
	static const char pbrVertexShaderSource[] =
		R"STR(
			#version 430 core
			...
			layout(std140,binding = 0,set = 0) uniform ViewProjectionBuffer
			{
				mat4 View;
				mat4 Projection;
			};
			...
			{
				gl_Position = vec4(aPos,1.0f)*transpose(Projection*View*Model);
				mat4 transposedModel = transpose(Model);

				vec3 T = normalize(vec3(transposedModel * vec4(aTangent,   0.0)));
				vec3 B = normalize(vec3(transposedModel * vec4(aBiTangent, 0.0)));
				vec3 N = normalize(vec3(transposedModel * vec4(aNormal,    0.0)));
				
				f_Normal = aNormal;
				f_TBN = mat3(T,B,N);
				f_Uv = aUv;
				vec3 worldPos = (vec4(aPos,1.0f)*transposedModel).xyz;
				f_WorldPos = worldPos;
			}
)STR";

	static const char pbrFragmentShaderSource[] =
		R"STR(
			...
			layout(binding = 0,set = 2) uniform texture2D albedoTexture;
			layout(binding = 1,set = 2) uniform texture2D normalTexture;
			layout(binding = 2,set = 2) uniform texture2D roughnessTexture;
			layout(binding = 3,set = 2) uniform texture2D metalnessTexture;

			layout(std140,binding = 0,set = 3) uniform CameraBuffer
			{
				vec3 ViewPos;
			};
			...
			void main()
			{
				...
				{
					vec3 irradiance = vec3(255 / 255.0f, 222 / 255.0f, 131 / 255.0f);
	...
	void Run()
	{
		...
		//Compile pbr shaders
		unsigned char* pPbrVertexShaderSPIRVBytes = nullptr;
		unsigned int pbrVertexShaderSPIRVByteCount = 0;
		std::string pbrVertexShaderCompilationErrors;
		DEV_ASSERT(ShaderCompiler::TextToSPIRV(pbrVertexShaderSource, "main", shaderc_vertex_shader, shaderc_source_language_glsl, &pPbrVertexShaderSPIRVBytes, pbrVertexShaderSPIRVByteCount, pbrVertexShaderCompilationErrors),"Main","Failed to compile pbr vertex shader with logs: %s", pbrVertexShaderCompilationErrors.c_str());

		unsigned char* pPbrFragmentShaderSPIRVBytes = nullptr;
		unsigned int pbrFragmentShaderSPIRVByteCount = 0;
		std::string pbrFragmentShaderCompilationErrors;
		DEV_ASSERT(ShaderCompiler::TextToSPIRV(pbrFragmentShaderSource, "main", shaderc_fragment_shader, shaderc_source_language_glsl, &pPbrFragmentShaderSPIRVBytes, pbrFragmentShaderSPIRVByteCount, pbrFragmentShaderCompilationErrors), "Main", "Failed to compile pbr fragment shader with logs: %s", pbrFragmentShaderCompilationErrors.c_str());

		//Create pbr shaders
		Shader* pPbrVertexShader = pDevice->CreateShader("main",VK_SHADER_STAGE_VERTEX_BIT, pPbrVertexShaderSPIRVBytes, pbrVertexShaderSPIRVByteCount);
		Shader* pPbrFragmentShader = pDevice->CreateShader("main",VK_SHADER_STAGE_FRAGMENT_BIT, pPbrFragmentShaderSPIRVBytes, pbrFragmentShaderSPIRVByteCount);

		//Compile shadow shaders
		unsigned char* pShadowVertexShaderSPIRVBytes = nullptr;
		unsigned int shadowVertexShaderSPIRVByteCount = 0;
		std::string shadowVertexShaderCompilationErrors;
		DEV_ASSERT(ShaderCompiler::TextToSPIRV(shadowVertexShaderSource, "main", shaderc_vertex_shader, shaderc_source_language_glsl, &pShadowVertexShaderSPIRVBytes, shadowVertexShaderSPIRVByteCount, shadowVertexShaderCompilationErrors), "Main", "Failed to compile shadow vertex shader with logs: %s", shadowVertexShaderCompilationErrors.c_str());

		unsigned char* pShadowFragmentShaderSPIRVBytes = nullptr;
		unsigned int shadowFragmentShaderSPIRVByteCount = 0;
		std::string shadowFragmentShaderCompilationErrors;
		DEV_ASSERT(ShaderCompiler::TextToSPIRV(shadowFragmentShaderSource, "main", shaderc_fragment_shader, shaderc_source_language_glsl, &pShadowFragmentShaderSPIRVBytes, shadowFragmentShaderSPIRVByteCount, shadowFragmentShaderCompilationErrors), "Main", "Failed to compile shadow fragment shader with logs: %s", shadowFragmentShaderCompilationErrors.c_str());

		//Create shadow shaders
		Shader* pShadowVertexShader = pDevice->CreateShader("main", VK_SHADER_STAGE_VERTEX_BIT, pShadowVertexShaderSPIRVBytes, shadowVertexShaderSPIRVByteCount);
		Shader* pShadowFragmentShader = pDevice->CreateShader("main", VK_SHADER_STAGE_FRAGMENT_BIT, pShadowFragmentShaderSPIRVBytes, shadowFragmentShaderSPIRVByteCount);
		...
		for (const MeshImportData& mesh : sponzaMeshes)
		{
			...
			applicationMemoryRequirement += sizeof(MeshViewProjectionBuffer) + sizeof(MeshModelBuffer);
		}
		...
		DirectionalLight* pDirectionalLight = new DirectionalLight(pDevice, pDeviceMemory, 4096, 4096);
		...
		//Create single texture layout
		const unsigned int singleTextureSetLayoutBindings[] = { 0 };
		const VkDescriptorType singleTextureSetLayoutDescriptorTypes[] = { VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE };
		const unsigned int singleTextureLayoutDescriptorCount[] = { 1 };
		const VkShaderStageFlags singleTextureLayoutShaderStages[] = { VK_SHADER_STAGE_FRAGMENT_BIT };
		
		DescriptorSetLayout* pSingleTextureSetLayout = pDevice->CreateDescriptorSetLayout(singleTextureSetLayoutBindings, singleTextureSetLayoutDescriptorTypes, singleTextureLayoutDescriptorCount, singleTextureLayoutShaderStages, 1);

		//Create descriptor pool
		const unsigned int poolDescriptorCounts[] = { sponzaMeshes.size() + 2000 ,(sponzeMaterials.size()*4) + 2000 ,20 };
		...
		DescriptorPool* pDescriptorPool = pDevice->CreateDescriptorPool(VkDescriptorPoolCreateFlags(), sponzeMaterials.size() + sponzaMeshes.size()*2 + 1000, poolDescriptorTypes, poolDescriptorCounts, 3);
		...
		GraphicsBuffer* pCameraBuffer = pDevice->CreateBuffer(pDeviceMemory, VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT, VK_SHARING_MODE_EXCLUSIVE, sizeof(FragmentCameraBuffer));
		GraphicsBuffer* pCameraStageBuffer = pDevice->CreateBuffer(pHostMemory, VK_BUFFER_USAGE_TRANSFER_SRC_BIT, VK_SHARING_MODE_EXCLUSIVE, sizeof(FragmentCameraBuffer));
		...
		pDevice->UpdateDescriptorSetBuffer(pStaticDescriptorSet, 0, 0, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, pCameraBuffer, 0, sizeof(FragmentCameraBuffer));
		...
		pDevice->UpdateDescriptorSetTextureSampler(pStaticDescriptorSet, 3, 0, VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE, pDirectionalLight->GetShaderTextureView(), nullptr, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);
		...
		pDevice->UpdateDescriptorSetTextureSampler(pDirectionalShadowTextureSet, 0, 0, VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE, pDirectionalLight->GetShaderTextureView(), nullptr, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);
		...
		for (GraphicsBuffer* pBuffer : textureTempBuffers)
		{
			delete pBuffer;
		}
		textureTempBuffers.clear();

		//delete pEnvironmentTextureStageBuffer;

		//Calculate scene bounding box and properties
		glm::vec3 sceneBoundingBoxMin = glm::vec3(0, 0, 0);
		glm::vec3 sceneBoundingBoxMax = glm::vec3(0, 0, 0);
		for (const MeshImportData& meshImportData : sponzaMeshes)
		{
			for (const VertexVector3D& pos : meshImportData.Positions)
			{
				sceneBoundingBoxMin.x = pos.X < sceneBoundingBoxMin.x ? pos.X /70.0f : sceneBoundingBoxMin.x;
				sceneBoundingBoxMin.y = pos.Y < sceneBoundingBoxMin.y ? pos.Y /70.0f : sceneBoundingBoxMin.y;
				sceneBoundingBoxMin.z = pos.Z < sceneBoundingBoxMin.z ? pos.Z /70.0f : sceneBoundingBoxMin.z;

				sceneBoundingBoxMax.x = pos.X > sceneBoundingBoxMax.x ? pos.X /70.0f : sceneBoundingBoxMax.x;
				sceneBoundingBoxMax.y = pos.Y > sceneBoundingBoxMax.y ? pos.Y /70.0f : sceneBoundingBoxMax.y;
				sceneBoundingBoxMax.z = pos.Z > sceneBoundingBoxMax.z ? pos.Z /70.0f : sceneBoundingBoxMax.z;
			}
		}
		const glm::vec3 sceneCenter = (sceneBoundingBoxMin + sceneBoundingBoxMax) / 2.0f;
		...
		for (const MeshImportData& meshImportData : sponzaMeshes)
		{
			...
			renderable.pViewProjectionStageBuffer = pDevice->CreateBuffer(pHostMemory, VK_BUFFER_USAGE_TRANSFER_SRC_BIT, VK_SHARING_MODE_EXCLUSIVE, sizeof(MeshViewProjectionBuffer));
			renderable.pModelStageBuffer = pDevice->CreateBuffer(pHostMemory, VK_BUFFER_USAGE_TRANSFER_SRC_BIT, VK_SHARING_MODE_EXCLUSIVE, sizeof(MeshModelBuffer));
			...
			renderable.pViewProjectionBuffer = pDevice->CreateBuffer(pDeviceMemory, VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT, VK_SHARING_MODE_EXCLUSIVE, sizeof(MeshViewProjectionBuffer));
			renderable.pModelBuffer = pDevice->CreateBuffer(pDeviceMemory, VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT, VK_SHARING_MODE_EXCLUSIVE, sizeof(MeshModelBuffer));
			...
			renderable.pViewportProjectionDescriptorSet = pDevice->AllocateDescriptorSet(pDescriptorPool, pRenderableSetLayout);
			renderable.pModelDescriptorSet = pDevice->AllocateDescriptorSet(pDescriptorPool, pRenderableSetLayout);

			//Update descriptor sets
			pDevice->UpdateDescriptorSetBuffer(renderable.pViewportProjectionDescriptorSet, 0, 0, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, renderable.pViewProjectionBuffer, 0, renderable.pViewProjectionBuffer->GetSize());
			pDevice->UpdateDescriptorSetBuffer(renderable.pModelDescriptorSet, 0, 0, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, renderable.pModelBuffer, 0, renderable.pModelBuffer->GetSize());
		...
		Pipeline* pPbrPipeline = nullptr;
		{
			...
			const Shader* pPipelineShaders[] = { pPbrVertexShader,pPbrFragmentShader };
			...
			DescriptorSetLayout* pSetLayouts[] = { pRenderableSetLayout,pRenderableSetLayout,pMaterialSetLayout,pStaticSetLayout };
			pPbrPipeline = pDevice->CreateGraphicsPipeline(
				vertexInputState,
				inputAssemblyState,
				nullptr,
				&viewportDesc,
				...
				nullptr,
				pPipelineShaders, 2,
				(const DescriptorSetLayout**)pSetLayouts, 4,
				nullptr,
				nullptr, 0,
				&renderingInfo);
		}
		...
		Pipeline* pShadowPipeline = nullptr;
		{
			VkPipelineVertexInputStateCreateInfo vertexInputState = {};
			vertexInputState.sType = VK_STRUCTURE_TYPE_PIPELINE_VERTEX_INPUT_STATE_CREATE_INFO;
			vertexInputState.flags = 0;
			...
			const Shader* pPipelineShaders[] = { pShadowVertexShader,pShadowFragmentShader };
			...
			viewport.x = 0;
			viewport.y = 0;
			viewport.width = pDirectionalLight->GetWidth();
			viewport.height = pDirectionalLight->GetHeight();
			viewport.minDepth = 0;
			viewport.maxDepth = 1.0f;

			VkRect2D scissor = {};
			scissor.offset = { 0,0 };
			scissor.extent = { pDirectionalLight->GetWidth(),pDirectionalLight->GetHeight() };
			...
			DescriptorSetLayout* pSetLayouts[] = { pRenderableSetLayout,pRenderableSetLayout };
			pShadowPipeline = pDevice->CreateGraphicsPipeline(
				vertexInputState,
				inputAssemblyState,
				nullptr,
				&viewportDesc,
				...
				nullptr,
				pPipelineShaders, 2,
				(const DescriptorSetLayout**)pSetLayouts, 2,
				nullptr,
				nullptr, 0,
				&renderingInfo);
		}
		...
		while (pWindow->IsActive())
		{
			...
			//Set shadow depth map
			{
				...
				pCmdList->SetDynamicRenderingDepthAttachment(pDirectionalLight->GetShaderTextureView(), VK_IMAGE_LAYOUT_DEPTH_ATTACHMENT_OPTIMAL, VkResolveModeFlags(), nullptr, VK_IMAGE_LAYOUT_UNDEFINED, VK_ATTACHMENT_LOAD_OP_CLEAR, VK_ATTACHMENT_STORE_OP_STORE, depthClearValue);
