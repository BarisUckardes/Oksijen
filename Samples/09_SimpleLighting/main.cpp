...
namespace Oksijen
{
	static const char vertexShaderSource[] =
		R"STR(
			...
			layout(location = 2) out mat3 TBNOut;
			...
			void main()
			{
				gl_Position = vec4(PosIn,1.0f)*Mvp;
				vec3 T = normalize(vec3(Model * vec4(TangentIn,   0.0)));
				vec3 B = normalize(vec3(Model * vec4(BiTangentIn, 0.0)));
	...
	static const char pixelShaderSource[] =
		R"STR(
			...
			layout(location = 1) in vec3 NormalIn;
			layout(location = 2) in mat3 TBNIn;
			layout(location = 0) out vec4 ColorOut;
			
			layout(binding = 1,set = 0) uniform texture2D albedoTexture;
			layout(binding = 2,set = 0) uniform texture2D normalTexture;
			layout(binding = 3,set = 0) uniform sampler sampler0;
			void main()
			{
				vec4 albedoColor = texture(sampler2D(albedoTexture,sampler0),UvIn);
				vec3 textureNormal = texture(sampler2D(normalTexture,sampler0),UvIn).xyz;
				vec3 normalizedTextureNormal = textureNormal *2.0f -1.0f;
				vec3 tbnNormal = normalize(TBNIn*normalizedTextureNormal);

				float directLight = dot(tbnNormal,vec3(0,-1,0));
				ColorOut = albedoColor*directLight;
			}
)STR";
	...
	void Run()
	{
		...
		windowDesc.Title = "09_SimpleLighting";
		...
		const unsigned long long requiredAppMemory =  meshVertexDataSize + meshIndexDataSize + albedoTextureDataSize +normalTextureDataSize + depthStencilTextureMemory + MB_TO_BYTE(1);
		...
		constexpr unsigned int descriptorTypeCounts[] = { 1,2,1 };
		...
		constexpr unsigned int descriptorBindingIndexes[4] = { 0,1,2,3 };
		constexpr VkDescriptorType descriptorLayoutTypes[4] = { VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER,VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE,VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE,VK_DESCRIPTOR_TYPE_SAMPLER };
		constexpr unsigned int descriptorLayoutDescriptorCounts[4] = { 1,1,1,1 };
		constexpr VkShaderStageFlags descriptorLayoutShaderStageFlags[4] = {VK_SHADER_STAGE_VERTEX_BIT, VK_SHADER_STAGE_FRAGMENT_BIT,VK_SHADER_STAGE_FRAGMENT_BIT,VK_SHADER_STAGE_FRAGMENT_BIT };
		...
		while (pWindow->IsActive())
		{
			...
			const UniformBufferData uniformBufferData = {mvp,model};
